...
import { Trash2, Plus, RefreshCw, Bot, ShieldCheck, MailWarning, Brain, Eraser, MessageSquareX, Settings2 } from "lucide-react"
...
export default function AccountsPage() {
  ...
  const handleAccountAction = async (
    ...
    try {
      const res = await fetch(`${API_BASE}/api/admin/accounts/${encodeURIComponent(targetEmail)}/${path}`, {
        method: "POST",
        headers: getAuthHeader(),
      })
      ...
      if (data.ok) {
        ...
        toast.error(data.body || data.error || fallbackError, { id, duration: 8000 })
      }
    } catch {
              ...
              <tr key={acc.email} className="transition-colors hover:bg-black/5 dark:hover:bg-white/5">
                ...
                <td className="px-6 py-4 align-middle text-right">
                  <div className="flex items-center justify-end gap-1 flex-wrap">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => handleDisableUpdateMemory(acc.email)} className="h-8 w-8 p-0" title={"关闭更新记忆"}>
                      <Settings2 className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleDisableMemory(acc.email)} className="h-8 w-8 p-0" title={"关闭记忆"}>
                      <Brain className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleClearMemories(acc.email)} className="h-8 w-8 p-0" title={"清空记忆"}>
                      <Eraser className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="sm" onClick={() => handleClearChats(acc.email)} className="h-8 w-8 p-0" title={"清空聊天记录"}>
