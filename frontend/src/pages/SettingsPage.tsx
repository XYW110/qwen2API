      ...
      <div className="grid gap-6 min-w-0">
        ...
        <div className="rounded-xl border bg-card text-card-foreground shadow-sm min-w-0">
          <div className="flex flex-col space-y-1.5 p-6 border-b bg-muted/30">
            ...
            <p className="text-sm text-muted-foreground">
              将已有的 API Key
              粘贴到此处，控制台将使用它进行所有的管理操作。（保存在浏览器本地）
            </p>
          </div>
          <div className="p-6">
            <div className="flex gap-2 items-center flex-wrap">
              ...
              <Button variant="ghost" onClick={handleClearSessionKey}>
                清除
              </Button>
            </div>
          </div>
        ...
        <div className="rounded-xl border bg-card text-card-foreground shadow-sm min-w-0">
          <div className="flex flex-col space-y-1.5 p-6 border-b bg-muted/30">
            ...
            <p className="text-sm text-muted-foreground">
              运行时并发槽位与排队阈值（需要在后端 config.json
              中修改后重启生效）。
            </p>
          </div>
          <div className="p-6 space-y-4">
            ...
            <div className="flex justify-between items-center py-2 border-b flex-wrap gap-4">
              <div className="space-y-1 min-w-0 flex-1">
                <span className="text-sm font-medium">
                  单账号最大并发 (max_inflight_per_account)
                </span>
                <p className="text-xs text-muted-foreground">
                  每个上游账号同时处理的请求数。太大易被封，太小不充分利用。
                </p>
              </div>
              <input
                type="number"
            ...
            <div className="flex justify-between items-center py-2 border-b flex-wrap gap-4">
              <div className="space-y-1 min-w-0 flex-1">
                <span className="text-sm font-medium">
                  全局并发上限 (global_max_inflight)
                </span>
                <p className="text-xs text-muted-foreground">
                  所有账号合计同时在途请求的硬上限。0 = 不限。对应 Dashboard
                  的"异步任务"峰值。
                </p>
              </div>
              <input
            ...
            <div className="flex justify-end">
              <Button size="sm" onClick={handleSaveConcurrency}>
                保存并发设置
              </Button>
            </div>
          </div>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="flex justify-between items-center py-2 border-b flex-wrap gap-4">
              <div className="space-y-1 min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  chat_id 超过此时长则丢弃重建，避免被上游静默回收。默认 10。
                </p>
              </div>
              <input
                type="number"
            ...
            <div className="flex justify-between items-center py-2 border-b flex-wrap gap-4">
              <div className="space-y-1 min-w-0 flex-1">
                <span className="text-sm font-medium">预热总数上限</span>
                <p className="text-xs text-muted-foreground">
                  所有账号所有模型的 chat_id 总数上限。超过此数停止补位。
                </p>
              </div>
              <input
                type="number"
                min="1"
                max="1000"
                value={maxTotalPrewarm}
                onChange={(e) => setMaxTotalPrewarm(Number(e.target.value))}
                className="flex h-8 w-20 rounded-md border border-input bg-background px-3 py-1 text-sm text-center"
              />
            </div>
            <div className="flex justify-end">
        ...
        <div className="rounded-xl border bg-card text-card-foreground shadow-sm min-w-0">
          <div className="flex flex-col space-y-1.5 p-6 border-b bg-muted/30">
            <h3 className="font-semibold leading-none tracking-tight">
              自动模型映射规则 (Model Aliases)
            </h3>
            <p className="text-sm text-muted-foreground">
              下游传入的模型名称将被网关自动路由至以下千问实际模型。请使用标准
              JSON 格式编辑。
            </p>
          </div>
          <div className="p-6">
