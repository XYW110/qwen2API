...
type MetricsData = {
  snapshot: {
    ...
    total_tokens: number;
    model_token_distribution: Record<string, { prompt: number; completion: number }>;
  };
  time_series: Array<{ time: string; qps: number; errors: number }>;
  token_time_series: Array<{ time: string; prompt_tokens: number; completion_tokens: number }>;
};

type ApiKeyStatsEntry = {
  api_key: string;
  model: string;
...
type ApiKeyRanking = {
  stats: ApiKeyStatsEntry[];
};

const tabs = [
  { id: "overview", label: "概览", icon: LayoutDashboard },
  { id: "performance", label: "性能趋势", icon: Activity },
  { id: "models", label: "模型分布", icon: PieChart },
  { id: "errors", label: "错误分析", icon: AlertTriangle },
  { id: "tokens", label: "Token 概览", icon: Coins },
  { id: "apikey", label: "API Key 排行", icon: KeyRound },
];
...
export default function Dashboard() {
  ...
  const [apiKeyStats, setApiKeyStats] = useState<ApiKeyRanking | null>(null);
  ...
  // 轮询 /api/admin/keys/stats
  useEffect(() => {
    const fetchApiKeyStats = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/admin/keys/stats`, {
          headers: getAuthHeader(),
        });
        if (res.ok) {
          const data = await res.json();
    ...
    fetchApiKeyStats();
    const timer = setInterval(fetchApiKeyStats, 5000);
    return () => clearInterval(timer);
  }, []);
    ...
    <div className="space-y-6 max-w-5xl relative">
        ...
        <div className="space-y-8">
          ...
          <div className="grid gap-6 md:grid-cols-2 relative z-10">
            <StatCard
              ...
              title="总请求数"
              value={String(snapshot?.total_requests ?? 0)}
              accent="primary"
              sub={`运行时长 ${Math.floor(
                (snapshot?.uptime_seconds ?? 0) / 60
              )} 分钟`}
            />
            <StatCard
              ...
              accent="blue"
              sub={`总错误 ${snapshot?.total_errors ?? 0}`}
            />
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 relative z-10">
            ...
            <StatCard
              ...
              title="当前并发"
              value={String(acc.in_use ?? 0)}
              accent="blue"
              sub={`全局上限 ${acc.global_in_use ?? 0}`}
            />
            <StatCard
              icon={<ShieldAlert className="h-5 w-5 text-destructive" />}
              title="排队请求"
              value={String(acc.waiting ?? 0)}
              accent="destructive"
              sub={`队列上限 ${acc.max_queue_size ?? 0}`}
            />
            <StatCard
              icon={<ActivityIcon className="h-5 w-5 text-orange-400" />}
              title="限流号/失效号"
              value={`${acc.rate_limited ?? 0} / ${acc.invalid ?? 0}`}
              accent="orange"
            />
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3 relative z-10">
            <StatCard
              ...
              sub={
                pool
                  ? `每账号目标 ${pool.target_per_account}  · TTL ${Math.round(
                      (pool.ttl_seconds || 0) / 60
                    )} 分钟`
                  : "未启用"
              }
            />
            <StatCard
              ...
              title="预热命中率"
              value={`${pool?.hit_rate ?? 0}%`}
              accent="amber"
              sub={`命中 ${pool?.hit_count ?? 0} / 总计 ${(pool?.hit_count ?? 0) + (pool?.miss_count ?? 0)}`}
            />
            <StatCard
              icon={<Database className="h-5 w-5 text-cyan-400" />}
              title="异步任务"
              value={String(status?.runtime?.asyncio_running_tasks ?? 0)}
              accent="cyan"
              sub="asyncio active task count"
            />
          </div>
                  ...
                  <tbody className="divide-y divide-border/40">
                    {rows.map((r) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-6 py-3 font-mono text-xs text-foreground/80 break-all">
                            {r.email}
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-bold ring-1 ${badge}`}
                            >
                              {r.status}
                            </span>
                          </td>
            ...
            <div className="p-0 relative z-10">
              <div className="divide-y divide-border/50 text-sm">
                ...
                <EndpointRow
                  ...
                  path="POST /v1/models/gemini-pro:generateContent"
                  tag="Gemini"
                  tagColor="yellow"
                />
                <EndpointRow
                ...
                <EndpointRow
                  icon={
                    <Paperclip className="h-5 w-5 text-cyan-500 dark:text-cyan-400" />
                  }
      ...
      {activeTab === "performance" && (
        <div className="space-y-8">
          <div className="relative z-10">
            ...
            <p className="text-muted-foreground mt-2 text-lg">
              QPS 趋势与延迟分布统计（每 3 秒自动刷新）。
            </p>
          </div>

          <div className="grid gap-6 lg:grid-cols-2 relative z-10">
            <QpsChart data={metrics?.time_series || []} />
            <LatencyChart
              p50={snapshot?.latency_p50 ?? 0}
              p95={snapshot?.latency_p95 ?? 0}
              p99={snapshot?.latency_p99 ?? 0}
            />
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3 relative z-10">
            <StatCard
              icon={<Activity className="h-5 w-5 text-blue-400" />}
              title="当前 QPS"
              value={String((snapshot?.qps ?? 0).toFixed(2))}
              accent="blue"
              sub="每秒请求数"
            />
            <StatCard
              ...
              title="P50 延迟"
              value={`${((snapshot?.latency_p50 ?? 0) * 1000).toFixed(0)} ms`}
              accent="primary"
              sub="中位数响应时间"
            />
            <StatCard
              ...
              title="P95 延迟"
              value={`${((snapshot?.latency_p95 ?? 0) * 1000).toFixed(0)} ms`}
              accent="orange"
              sub="95 百分位响应时间"
            />
          </div>
      ...
      {activeTab === "models" && (
        <div className="space-y-8">
          ...
          <ModelPieChart data={snapshot?.model_distribution || {}} />

          {/* 可选：Top 账号使用统计 */}
          {snapshot && Object.keys(snapshot.model_distribution).length > 0 && (
            <div className="rounded-2xl border border-border/50 bg-card/30 backdrop-blur-xl shadow-2xl relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-b from-black/[0.02] dark:from-white/[0.02] to-transparent pointer-events-none" />
              <div className="flex flex-col space-y-2 p-6 border-b border-border/50 bg-muted/10 relative z-10">
                <h3 className="font-extrabold text-xl tracking-tight flex items-center gap-3">
        ...
        <div className="space-y-8">
          ...
          <div className="grid gap-6 md:grid-cols-3 relative z-10">
            <StatCard
              ...
              title="总错误数"
              value={String(snapshot?.total_errors ?? 0)}
              accent="destructive"
              sub="累计错误请求"
            />
            <StatCard
              ...
              title="错误率"
              value={`${(100 - (snapshot?.success_rate ?? 100)).toFixed(2)}%`}
              accent="orange"
              sub="失败请求占比"
            />
            <StatCard
              icon={<TrendingUp className="h-5 w-5 text-emerald-400" />}
              title="成功率"
              value={`${(snapshot?.success_rate ?? 100).toFixed(2)}%`}
              accent="primary"
              sub="成功请求占比"
            />
          </div>

          <ErrorBarChart data={snapshot?.status_code_distribution || {}} />
        </div>
      )}
        ...
        <div className="space-y-8">
          ...
          <div className="grid gap-6 md:grid-cols-3 relative z-10">
            <StatCard
              ...
              title="总 Token 消耗"
              value={formatTokenCount(snapshot?.total_tokens ?? 0)}
              accent="blue"
              sub={`Prompt ${formatTokenCount(snapshot?.total_prompt_tokens ?? 0)} + Completion ${formatTokenCount(snapshot?.total_completion_tokens ?? 0)}`}
            />
            <StatCard
              icon={<Activity className="h-5 w-5 text-violet-400" />}
              title="Prompt Tokens"
              value={formatTokenCount(snapshot?.total_prompt_tokens ?? 0)}
              accent="primary"
              sub="输入 Token 累计"
            />
            <StatCard
              ...
              value={formatTokenCount(snapshot?.total_completion_tokens ?? 0)}
              accent="primary"
              sub="输出 Token 累计"
            />
          </div>
          ...
          {/* Token 时序图 + 模型分布图 */}
          <div className="grid gap-6 lg:grid-cols-2 relative z-10">
            <TokenAreaChart data={metrics?.token_time_series || []} />
            <TokenModelBarChart data={snapshot?.model_token_distribution || {}} />
          </div>
        </div>
      )}
      ...
      {activeTab === "apikey" && (
        <div className="space-y-8">
          <div className="relative z-10">
            ...
            <p className="text-muted-foreground mt-2 text-lg">
              按 API Key 的调用与 Token 消耗排行（每 5 秒自动刷新）。
            </p>
          </div>

          {/* 排行榜表格 */}
          <ApiKeyRankingTable stats={apiKeyStats?.stats || []} />
        </div>
      )}
    </div>
  );
...
function StatCard({
  ...
  return (
    <div
      className={`group rounded-2xl border border-border/50 bg-card/40 backdrop-blur-md shadow-xl ${shadowMap[accent]} transition-all duration-500 overflow-hidden relative`}
    >
      <div
        className={`absolute inset-0 bg-gradient-to-br ${gradMap[accent]} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
      />
      <div className="p-6 relative z-10">
        <div className="flex flex-row items-center justify-between space-y-0 pb-4">
          <h3 className="tracking-tight text-sm font-semibold text-foreground/80 uppercase">
            {title}
          </h3>
...
function EndpointRow({
  ...
  return (
    <div className="flex justify-between items-center px-8 py-5 hover:bg-black/5 dark:hover:bg-white/[0.02] transition-colors">
      ...
      <span
        className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-bold ${tagClass}`}
      >
        {tag}
      </span>
    </div>
...
// API Key 排行表格组件
function ApiKeyRankingTable({ stats }: { stats: ApiKeyStatsEntry[] }) {
  // 按 api_key 聚合：每个 key 的 total_requests 和 total_tokens
  const aggregated = new Map<string, { key: string; totalRequests: number; totalTokens: number; models: number }>();
  for (const entry of stats) {
    const existing = aggregated.get(entry.api_key);
    if (existing) {
      existing.totalRequests += entry.request_count;
      existing.totalTokens += entry.total_tokens;
      existing.models += 1;
    } else {
      aggregated.set(entry.api_key, {
        key: entry.api_key,
  ...
  const ranking = Array.from(aggregated.values())
    .sort((a, b) => b.totalTokens - a.totalTokens); // 默认按 Token 降序
  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-card/30 backdrop-blur-xl shadow-2xl relative overflow-hidden">
      ...
      <div className="flex flex-col space-y-2 p-6 border-b border-border/50 bg-muted/10 relative z-10">
        ...
        <p className="text-sm text-muted-foreground ml-5">按总 Token 消耗降序排列，API Key 脱敏显示</p>
      </div>
      <div className="overflow-x-auto relative z-10">
        {ranking.length > 0 ? (
          <table className="w-full text-sm">
            <thead className="bg-muted/20 text-xs uppercase text-muted-foreground">
              <tr>
                <th className="text-left px-6 py-3 font-semibold">排名</th>
                <th className="text-left px-6 py-3 font-semibold">API Key</th>
                <th className="text-right px-4 py-3 font-semibold">总请求数</th>
                <th className="text-right px-4 py-3 font-semibold">总 Token 数</th>
                <th className="text-right px-4 py-3 font-semibold">使用模型数</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/40">
              {ranking.map((row, idx) => (
                <tr key={row.key} className="hover:bg-muted/10 transition-colors">
                  ...
                  <td className="px-6 py-3 font-mono text-xs text-foreground/80">
                    {/* 脱敏显示：显示前 8 位 + "..." */}
