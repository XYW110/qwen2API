...
import { Plus, RefreshCw, Copy, Check, Trash2, BarChart3, Pencil, X, ChevronUp } from "lucide-react"
...
export default function TokensPage() {
  ...
  const handleShowStats = (key: string) => {
    ...
    fetch(`${API_BASE}/api/admin/keys/stats/${encodeURIComponent(key)}`, { headers: getAuthHeader() })
      .then(res => {
          ...
          <div className="bg-card rounded-lg p-6 w-full max-w-md shadow-xl border">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={() => handleGenerate(generateNote || undefined)}>生成</Button>
              </div>
            </div>
          </div>
        </div>
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-muted/50 border-b text-muted-foreground">
            <tr>
              ...
              <th className="h-12 px-4 align-middle font-medium w-32">备注</th>
              <th className="h-12 px-4 align-middle font-medium w-40">创建时间</th>
              <th className="h-12 px-4 align-middle font-medium text-right w-32">操作</th>
            </tr>
          </thead>
          <tbody>
            {keys.length === 0 && (
              <tr>
                <td colSpan={5} className="p-4 text-center text-muted-foreground">暂无 API Key</td>
              </tr>
            )}
            {keys.map((item, i) => (
              <>
                <tr key={item.key} className="border-b transition-colors hover:bg-muted/50">
                  ...
                  <td className="p-4 align-middle font-mono text-xs">{item.key}</td>
                    ...
                    {editingNote === item.key ? (
                      <div className="flex gap-1 items-center">
                        <input
                          ...
                          autoFocus
                        />
                        <Button variant="ghost" size="sm" onClick={() => handleUpdateNote(item.key, noteValue || undefined)}>
                          <Check className="h-3 w-3 text-green-600" />
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => { setEditingNote(null); setNoteValue(""); }}>
                          <X className="h-3 w-3" />
                        </Button>
                      </div>
                    ) : (
                      <div
                        ...
                        onClick={() => { setEditingNote(item.key); setNoteValue(item.note || ""); }}
                      >
                  ...
                  <td className="p-4 align-middle text-xs text-muted-foreground">{formatDate(item.created_at)}</td>
                  <td className="p-4 align-middle text-right space-x-1">
                    <Button variant="ghost" size="sm" onClick={() => handleShowStats(item.key)} title="查看统计">
                      {statsKey === item.key ? <ChevronUp className="h-4 w-4" /> : <BarChart3 className="h-4 w-4" />}
                    </Button>
                    ...
                    <Button variant="ghost" size="sm" onClick={() => handleDelete(item.key)} className="text-destructive hover:bg-destructive/10 hover:text-destructive">
                  ...
                  <tr className="bg-muted/30 border-b">
                    <td colSpan={5} className="p-4">
                      <div className="text-xs">
                        <div className="font-medium mb-2">使用统计（按模型聚合）</div>
                        {statsData.length === 0 ? (
                          <div className="text-muted-foreground">暂无统计数据</div>
                        ) : (
                          <div className="grid gap-2">
                            {statsData.map((stat, idx) => (
                              <div key={idx} className="flex items-center gap-4 bg-background/50 rounded px-3 py-2">
                                <span className="font-medium">{stat.model}</span>
                                <span className="text-muted-foreground">请求: {stat.request_count}</span>
                                <span className="text-muted-foreground">Tokens: {stat.total_tokens}</span>
                              </div>
                            ))}
                          </div>
                        )}
